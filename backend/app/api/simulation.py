...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    try:
        ...
        logger.debug(
            f"Checking simulation prepared state: {simulation_id}, "
        ...
        if status in prepared_statuses and config_generated:
            ...
            logger.info(
                f"Simulation {simulation_id} check result: prepared "
                f"(status={status}, config_generated={config_generated})"
            )
            return True, {
                "status": status,
            ...
            logger.warning(
                f"Simulation {simulation_id} check result: not prepared "
                f"(status={status}, config_generated={config_generated})"
            )
            return False, {
                "reason": (
                    f"Status is not in the prepared-state list or config_generated is false: "
                    f"status={status}, config_generated={config_generated}"
                ),
                "status": status,
                "config_generated": config_generated
...
def prepare_simulation():
    ...
    try:
        ...
        logger.info(
            f"Starting /prepare request: simulation_id={simulation_id}, "
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": "Preparation has already been completed; no regeneration is needed",
            ...
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": "Preparation has already been completed",
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
                ...
                if is_prepared:
                    return jsonify({
                        ...
                        "data": {
                            ...
                            "status": "ready",
                            "progress": 100,
                            "message": "Task completed (preparation already existed)",
                            "already_prepared": True,
                            "prepare_info": prepare_info
                        }
                    })
    ...
    try:
        ...
        if state.status != SimulationStatus.READY:
            ...
            if is_prepared:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": (
                                    "Simulation is currently running. Call /stop first, "
                                    "or use force=true to restart anyway"
                                )
                            }), 400
                ...
                logger.info(
                    f"Simulation {simulation_id} preparation is complete; resetting status to ready "
                ...
                return jsonify({
                    "success": False,
                    "error": (
                        f"Simulation is not ready. Current status: {state.status.value}. "
                        "Call /prepare first"
                    )
                }), 400
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": (
                        "A valid graph_id is required when graph memory updates are enabled. "
                        "Make sure the project graph has been built"
                    )
                }), 400
        ...
        if not os.path.exists(db_path):
            return jsonify({
                ...
                "data": {
                    ...
                    "count": 0,
                    "posts": [],
                    "message": "Database does not exist yet; the simulation may not have run"
                }
            })
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                ...
                "error": (
                    ...
                    "Make sure the simulation has completed and entered command-wait mode."
                )
            }), 400
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": f"Interview list item {i+1} has an invalid platform; only 'twitter' or 'reddit' are allowed"
                }), 400
